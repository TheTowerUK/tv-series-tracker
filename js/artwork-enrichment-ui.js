(function artworkEnrichmentUiModule(root, factory) {
  ...
  function createArtworkEnrichmentReview({ document, discovery, artworkEngine, auth = null, getShows, getAuthority, candidateView } = {}) {
    if (!document || !discovery || typeof discovery.discover !== "function" || typeof getShows !== "function" ||
        typeof getAuthority !== "function" || !artworkEngine || typeof artworkEngine.hasArtwork !== "function" ||
    ...
    const els = {
      ...
      done: byId("closeArtworkReviewBtn")
    };
    ...
    function updateEntryPoint() {
      ...
      els.action.disabled = running || missing === 0;
    }
    ...
    function renderConfident(item) {
      ...
      const text = document.createElement("span"); text.textContent = "Keep this proposal selected for the future apply step";
    ...
    function renderSelectedSummary() {
      ...
      els.selected.textContent = `${count} artwork selection${count === 1 ? "" : "s"} ready for a future apply step. No tracker data has changed.`;
    }
    ...
    function reset() {
      ...
      running = false; abortController = null; session = null; selections.clear(); els.results.replaceChildren();
    ...
    return Object.freeze({ getSelectedProposals: () => session ? selectedProposals(session.results, selections) : Object.freeze([]), refreshAvailability: updateEntryPoint, reset, start });
  }

  return Object.freeze({ countsFor, createArtworkEnrichmentReview, posterUrl, proposal, safeErrorMessage, selectedProposals });
});
